...
{
    /// <summary>
    /// Contains unit tests for the StatusStateExtensions and StatusStateConverter classes.
    /// </summary>
    [Collection("NonParallelTests")]
    public class UnitTestStatusState
    {
        /// <summary>
        ...
        {
            // act
            var result = StatusState.Active.IsActive();
        ...
        {
            // act
            var result = StatusState.Archived.IsActive();
        ...
        /// </summary>
        [Theory]
        [InlineData(StatusState.Active)]
        [InlineData(StatusState.Archived)]
        public void Id_ReturnsNonEmptyGuid(StatusState state)
        {
            // act
            var id = state.Id();
        ...
        {
            // act
            var activeId = StatusState.Active.Id();
            var archivedId = StatusState.Archived.Id();
        ...
        {
            // act
            var id1 = StatusState.Active.Id();
            var id2 = StatusState.Active.Id();
        ...
        [InlineData(StatusState.Active)]
        [InlineData(StatusState.Archived)]
        public void Text_ReturnsNonNullString(StatusState state)
        {
            // act
            var text = state.Text();
        ...
        [InlineData(StatusState.Active)]
        [InlineData(StatusState.Archived)]
        public void Color_ReturnsNonNullString(StatusState state)
        {
            // act
        ...
        {
            // arrange
            var converter = new StatusStateConverter();

            // act
            var result = converter.FromRaw(null, typeof(StatusState));
        ...
        public void Converter_FromRaw_ReturnsActive_ForActiveGuid()
        {
            // arrange
            var converter = new StatusStateConverter();
            var rawValue = StatusState.Active.Id().ToString();

            // act
            var result = converter.FromRaw(rawValue, typeof(StatusState));

            // validation
            Assert.Equal(StatusState.Active, result);
        }
        ...
        public void Converter_FromRaw_ReturnsArchived_ForArchivedGuid()
        {
            // arrange
            var converter = new StatusStateConverter();
            var rawValue = StatusState.Archived.Id().ToString();

            // act
            var result = converter.FromRaw(rawValue, typeof(StatusState));

            // validation
            Assert.Equal(StatusState.Archived, result);
        }
        ...
        public void Converter_ToRaw_ReturnsActiveGuid_ForActiveState()
        {
            ...
            // act
            var result = converter.ToRaw(StatusState.Active, typeof(StatusState));

            // validation
            Assert.Equal(StatusState.Active.Id(), result);
        }
        ...
        public void Converter_ToRaw_ReturnsArchivedGuid_ForArchivedState()
        {
            ...
            // act
            var result = converter.ToRaw(StatusState.Archived, typeof(StatusState));

            // validation
            Assert.Equal(StatusState.Archived.Id(), result);
        }
    }
}
