...
{
    /// <summary>
    /// Provides methods to convert between raw data representations and strongly typed identity state objects for use in RESTful
    /// operations.
    /// </summary>
    public class IdentityStateConverter : IRestValueConverter
    {
        /// <summary>
        ...
        public object FromRaw(object rawValue, Type targetType)
        {
            ...
            if (rawValue is string s)
            {
                ...
                if (id == IdentityState.Locked.Id())
                {
                    return IdentityState.Locked;
                }

                if (id == IdentityState.Disabled.Id())
                {
        ...
        public object ToRaw(object value, Type sourceType)
        {
            return value switch
            {
                IdentityState.Locked => IdentityState.Locked.Id(),
                IdentityState.Disabled => IdentityState.Disabled.Id(),
                _ => IdentityState.Active.Id()
            };
        }
    }
}
