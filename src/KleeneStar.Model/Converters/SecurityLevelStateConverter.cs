...
{
    /// <summary>
    /// Provides methods to convert between raw data representations and strongly typed security
    /// level state objects for use in RESTful operations.
    /// </summary>
    public class SecurityLevelStateConverter : IRestValueConverter
    {
        /// <summary>
        ...
        public object FromRaw(object rawValue, Type targetType)
        {
            ...
            if (rawValue is string s)
            {
                ...
                if (id == SecurityLevelState.Archived.Id())
                {
                    return SecurityLevelState.Archived;
                }

                return SecurityLevelState.Active;
            }
        ...
        public object ToRaw(object value, Type sourceType)
        {
            return value switch
            {
                SecurityLevelState.Archived => SecurityLevelState.Archived.Id(),
                _ => SecurityLevelState.Active.Id()
            };
        }
    }
}
