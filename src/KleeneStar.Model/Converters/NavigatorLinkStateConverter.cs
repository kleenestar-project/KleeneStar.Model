...
{
    /// <summary>
    /// Provides methods to convert between raw data representations and strongly typed navigator link
    /// state objects for use in RESTful operations.
    /// </summary>
    public class NavigatorLinkStateConverter : IRestValueConverter
    {
        /// <summary>
        ...
        public object FromRaw(object rawValue, Type targetType)
        {
            ...
            if (rawValue is string s)
            {
                ...
                if (id == NavigatorLinkState.Hidden.Id())
                {
                    return NavigatorLinkState.Hidden;
                }

                return NavigatorLinkState.Active;
            }
        ...
        public object ToRaw(object value, Type sourceType)
        {
            return value switch
            {
                NavigatorLinkState.Hidden => NavigatorLinkState.Hidden.Id(),
                _ => NavigatorLinkState.Active.Id()
            };
        }
    }
}
